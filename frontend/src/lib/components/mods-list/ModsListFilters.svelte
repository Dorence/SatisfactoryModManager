...
<div class="px-5 py-1 flex @container/mod-list-filters items-center">
  <div class="w-0 grow mr-2 inline-flex items-center">
    <input class="w-full bg-transparent border-b-[1px] pr-5 border-secondary-500 hover:border-secondary-50 focus:border-primary-600 duration-500 placeholder-secondary-400 placeholder:font-medium outline-none !ring-0 h-8 transition-colors" placeholder={$t('mod-filter.search-placeholder')} bind:value={$search}/>
    <button
  ...
  >
    <svelte:fragment slot="selected" let:item>
      ...
      <span class="text-primary-600 hidden @lg/mod-list-filters:!block w-24 text-left">{$t('mod-filter.filter.' + item.name)}</span>
    </svelte:fragment>
    <svelte:fragment slot="item" let:item>
      <span>{$t('mod-filter.filter.' + item.name)}</span>
    </svelte:fragment>
  </Select>
  <Select
    name="modsOrderBy"
  ...
  >
    <svelte:fragment slot="selected" let:item>
      ...
      <span class="text-primary-600 hidden @lg/mod-list-filters:!block w-24 text-left">{$t('mod-filter.order.' + item.name)}</span>
    </svelte:fragment>
    <svelte:fragment slot="item" let:item>
      <span>{$t('mod-filter.order.' + item.name)}</span>
    </svelte:fragment>
  </Select>
</div>
